...
## With the dataframe of the model where I can see the confidence level of each classification.
## In pictures with more than one detection (both for the model and the manual validation) I will need to check it manually.
## But to save some time I will create a dataframe of pictures that both the model and the manual validation detected only one lizard (because than I can merge between the dataframes and for each filename there will be one row only)
...
setwd("D:/OfirL6/Documents/Ben/R/R/github/confidence thresholds/")
...
write.csv(model_and_validation_result_07.05.single_count, file = paste0("model_and_validation_result_07.05.single_count_",format(today(), "%d.%m.%y"),".csv"), row.names = FALSE)
...
write.csv(model_result_07.05.multiple_count, file = paste0("model_result_07.05.multiple_count_",format(today(), "%d.%m.%y"),".csv"), row.names = FALSE)
write.csv(validation_07.05_result.multiple_count, file = paste0("validation_07.05_result.multiple_count_",format(today(), "%d.%m.%y"),".csv"), row.names = FALSE)
