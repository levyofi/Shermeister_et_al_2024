...
class GradCam():
    ...
    def from_interp(cls,learn,interp,img_idx,ds_type=DatasetType.Valid,include_label=False):
        ...
        hmap_pred,xb_grad_pred = get_grad_heatmap(learn,xb,pred_idx,size=np.max(xb_img.shape))
...
def show_heatmap(hm,xb_im,size,ax=None):
    if ax is None:
        _,ax = plt.subplots()
    xb_im.show(ax)
    ax.imshow(hm, alpha=0.5, extent=(0,size,size,0),
...
def plot_combined(gcam1, gcam2, plot_hm=True):
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    # Plot original image
    size = gcam1.xb_img.shape[-1]
    gcam1.xb_img.show(axes[0])
    axes[0].set_axis_off()

    # Plot heatmap for model 1
    if plot_hm:
        show_heatmap(gcam1.hmap1, gcam1.xb_img, size, axes[1])
        axes[1].set_axis_off()

    # Plot heatmap for model 2
    if plot_hm:
        show_heatmap(gcam2.hmap1, gcam2.xb_img, size, axes[2])
        axes[2].set_axis_off()
