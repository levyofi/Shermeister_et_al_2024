# This code is used for classification of lizard's thermoregulation (sun or shade selection).
# The code was adapted from https://github.com/microsoft/computervision-recipes/blob/staging/scenarios/classification/12_hard_negative_sampling.ipynb
...
DATA_PATH = "Data/thermoregulation" 
...
# To enable hard mining on the "negative" class (the "helper" class where the detection model generated a false positive prediction) we explicitly define its path that will be used later
neg_datapath = Path("Data/thermoregulation/negative")
...
for i in range(0,20):
   ...
   # plt.show()
    ...
    data = (ImageList.from_folder(datapath)
            ...
            .normalize(imagenet_stats))
    print(data.batch_stats)
    learn.save("lizard_model_thermoergulation_"+str(i)+"_"+f"{acc:4.2f}")
    learn.export("lizard_model_thermoergulation_"+str(i)+"_"+f"{acc:4.2f}"+".pkl")
    # Set the updated dataset to the learner 
    learn.data = data
