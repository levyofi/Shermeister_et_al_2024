# This code is used for classification of lizard's marking color.
...
# To enable hard mining on the "unclear" class (the "helper" class where the color in unclear) we explicitly define its path that will be used later
neg_datapath = Path("Data/color/unclear")
# We split positive samples into 80% training and 20% validation
data_imlist = (
    ImageList.from_folder(ori_datapath)
    .split_by_rand_pct(valid_pct=0.3, seed=10)
    .label_from_folder()
)
...
for i in range(0,20):
    ...
    data = (ImageList.from_folder(datapath)
            ...
            .normalize(imagenet_stats))
    print(data.batch_stats)
    learn.save("lizard_model_color"+str(i)+"_"+f"{acc:4.2f}")
    learn.export("lizard_model_color"+str(i)+"_"+f"{acc:4.2f}"+".pkl")
